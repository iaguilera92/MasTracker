...
const FILE_KEY = "PruebaNodeRed_vehiculo.xlsx";
const LOCAL_PATH = path.resolve(__dirname, "PruebaNodeRed_vehiculo.xlsx");
...
exports.handler = async function (event) {
    console.log("📥 Petición recibida en restaurarVehiculos.cjs");
    ...
    try {
        ...
        console.log("✅ Excel de vehículos restaurado exitosamente desde local a S3");

        return {
            statusCode: 200,
            headers,
            body: JSON.stringify({ message: "Se ha restaurado el Excel de vehículos correctamente!" }),
        };
    } catch (error) {
        console.error("❌ Error al restaurar vehículos:", error);
        return {
            statusCode: 500,
            headers,
